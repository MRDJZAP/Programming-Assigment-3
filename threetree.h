...
class Node {
public:
  Node(pair<int, int> ul, int w, int h, RGBAPixel average,
       double v);                                  // constructor
...
public:
   ...
   * of the node's rectangular region into three approximately equal-size
   * strips.
   *
   ...
   *
   * If the dimensions are (3p+2) x q, subregions A and C each get an extra
   * pixel of size, while subregion B has dimensions p x q.
   *
   * If the region to be divided is a square, then apply the Split wide
   * behaviour.
   *
   ...
   * pointer A refers to the left / top subtree (i.e. smallest x / y image
   * coordinates) pointer B refers to the middle subtree pointer C refers to the
   * right / bottom subtree (i.e. largest x / y image coordinates)
   *
